...
in memory with disk-based persistence. All methods are best-effort —
...
_CACHE_DIR = Path(os.environ.get("XDG_CACHE_HOME", Path.home() / ".cache")) / "ai-cli-runner"
...
class LLMPricingCache:
        ...
        try:
            if _CACHE_FILE.exists():
                ...
                if isinstance(data, dict):
                    return data
                logger.warning("Disk cache is not a dict, ignoring")
        except Exception:
            logger.debug("Failed to read disk cache", exc_info=True)
        return None

    def _write_disk_cache(self, data: dict[str, Any]) -> None:
        """Write pricing data to disk cache. Best-effort.
        ...
        try:
            ...
            logger.debug("Wrote pricing cache to disk: %s", _CACHE_FILE)
        except Exception:
            logger.debug("Failed to write disk cache", exc_info=True)

    def _is_disk_cache_fresh(self) -> bool:
        """Check if disk cache exists and is less than 24 hours old."""
        try:
            if _CACHE_FILE.exists():
    ...
    async def _fetch(self) -> None:
        ...
        try:
            ...
            if self._is_disk_cache_fresh():
                ...
                if data is not None:
                    self._data = data
                    logger.debug("LLM pricing loaded from disk cache: %d models", len(self._data))
                    return
            ...
            # Fallback: stale disk cache
            data = self._read_disk_cache()
            if data is not None:
                self._data = data
                logger.debug("LLM pricing loaded from stale disk cache: %d models", len(self._data))

        except Exception:
            logger.warning("Failed to fetch LLM pricing data", exc_info=True)

    _CURSOR_ROUTING_SUFFIXES: ClassVar[tuple[str, ...]] = ("-xhigh-fast", "-fast", "-xhigh", "-max-thinking")
    _KNOWN_MODEL_PREFIXES: ClassVar[tuple[str, ...]] = ("claude", "gpt", "gemini")
    _KNOWN_VARIANTS: ClassVar[frozenset[str]] = frozenset({"opus", "sonnet", "haiku", "pro", "flash", "nano", "mini"})
    ...
    def _lookup_model(self, provider: str, model: str) -> dict[str, Any] | None:
        ...
        litellm_provider = self._PROVIDER_MAP.get(provider, provider)

        def _try_candidates(name: str) -> dict[str, Any] | None:
